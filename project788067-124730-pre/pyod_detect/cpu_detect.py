...
class CpuDetect():
    def detect(self, begin, end):
        ...
        y = npy.array(df['CPU%']).reshape(-1, 1)
        ...
        IX2 = npy.array(dfx['CPU%'][dfx['outlier']==0]).reshape(-1, 1)

        OX1 = npy.array(dfx['TICKS'][dfx['outlier']==1]).reshape(-1, 1)
        OX2 = npy.array(dfx['CPU%'][dfx['outlier']==1]).reshape(-1, 1)
        ...
        plt.savefig('cpu_' + (time.strftime('%Y-%m-%d_%H-%M-%S', time.localtime(df['TICKS'][i]))) + ".png")
